...
class wlb_branding {
	...
	function filter_wp_mail($args){		
		if( preg_match("/^from:/mi",$args['headers']) ){
...
//			),
	...
	function admin_head(){
		...
		$vars = array('dashboard_icon','header_logo','header_logo_link','header_logo_width','header_bar_height','developer_logo','developer_name','developer_url','hide_update_nag','hide_update_download','hide_contextual_help','hide_screen_options','hide_favorite_actions','remove_wordpress_from_title','remove_footer_version');
		foreach($vars as $var){
...
<?php	
		$this->handle_wp_logo_in_admin_bar($header_logo,$header_logo_width,$header_logo_link);
	}	
	
	function wp_head(){
		global $wlb_plugin;
		if(  '1'!=$wlb_plugin->get_option('use_branding_customization') )return true;
		$vars = array('header_logo','header_logo_width','header_logo_link');
		foreach($vars as $var){
			$$var = $wlb_plugin->get_option($var);
			if(in_array($var,array('header_logo'))){
				$$var = str_replace("{pluginurl}", WLB_URL, $$var);
	...
	function handle_wp_logo_in_admin_bar($header_logo,$header_logo_width,$header_logo_link=''){
?>
...
<?php if(trim($header_logo)!=''):?>
	...
	$('#wp-admin-bar-wp-logo > a.ab-item').attr('href','<?php echo $header_logo_link?>');
	<?php endif;?>
<?php endif; ?>
});
</script>
