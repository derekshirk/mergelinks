...
//			),
			...
			'cs_menu_main_color' 		=> (object)array(
				...
				'description' 	=> '',
				'style'			=> "#adminmenuback, #adminmenuwrap {background-color: {color};}",
				'subtitle'		=> __('Menu color customization','wlb')/*,
				'preview'		=> 	(object)array(
										'type'=>'preview',
										'path'=>WLB_URL.'images/preview/color_scheme/',
										'items'=>array(
											(object)array(
												'src'=> 'cs_menu_main_color.png',
												'focus_target'=>'#cs_menu_main_color'
											),
											(object)array(
												'src'=> 'cs_menu_link_color.png',
												'focus_target'=>'#cs_menu_link_color'
											)											
										)
									)*/
			),
			'cs_menu_main_color_hover'		=> (object)array(
				...
				'description' 	=> '',
				'style'			=> "#adminmenu li.menu-top:hover > a, #adminmenu li.menu-top.focused > a, #adminmenu li.menu-top > a:focus {background-color: {color};}",
			...
			'cs_menu_link_color'		=> (object)array(
				...
				'description' 	=> '',
				'style'			=> "#adminmenu a.menu-top {color: {color};}",
				'alternative'	=> "cs_menu_main_color",
				'variation'		=> -30
			),
...
//			),
			...
			*/
			'cs_submenu_bg_color'	=> (object)array(
				...
				'description' => '',
				'style'			=> "#adminmenu .wp-submenu ul {background-color: {color};}",
				'alternative'	=> "cs_menu_main_color",
				'variation'		=> 25
			),
...
//CONTENT AREA----
			'cs_bg_color'	=> (object)array(
				...
				'description'=> '',
				'style'	=> "html, .wp-dialog, #post-body {background-color:{color};}",
				'alternative'	=> "cs_menu_main_color",
				'variation'		=> 25			
			),
...
//THE EDITOR,	
	...
	function get_icon_sets($r = array()){
		...
		//--
		$icons_path = WLB_PATH.'resources/icons_sets'; 
			...
			if(is_array($arr)&&count($arr)==2){
				if($arr[0]=='core'){
					echo sprintf('<link rel="stylesheet" href="%s" type="text/css" media="all" />',
						WLB_URL.'resources/icons_sets/'.$arr[1].'/style.css'
					);
				}else if($arr[0]=='dc'){
					$upload_dir = wp_upload_dir();
					echo sprintf('<link rel="stylesheet" href="%s" type="text/css" media="all" />',
...
#adminmenu .wp-menu-arrow div {background-image:url(<?php echo WLB_URL.'css/images/wlb-menu-arrow-frame.png'?>) !important;}
...
#adminmenu .wp-menu-arrow div {background-image:url(<?php echo WLB_URL.'css/images/wlb-menu-arrow-frame-rtl.png'?>) !important;}
#adminmenu .wp-menu-arrow div:after {
	content:"";
	top:0px;
	display:block;
...
<?php	
	...
	function css_secondary_button($grad1,$grad2,$border,$font,$shadow,$h_border,$h_font,$disabled_bg,$disabled_border,$disabled_color){
		if(in_array(trim($grad1),array('','#')))return;
?>
a.button, a.button-secondary {	color: <?php echo $font?>;}
a.button:hover, a.button-secondary:hover {color: <?php echo $h_font?>;}
.button, .submit input, .button-secondary {text-shadow:0 1px 1px <?php echo $this->get_rgba($shadow,0.7)?>;}
.button, .submit input, .button-secondary
.button-disabled, .button-secondary[disabled], .button-secondary-disabled {
	background: none repeat scroll 0 0 <?php echo $disabled_bg?> !important;
	color:<?php echo $disabled_color?> !important;
	border-color:<?php echo $disabled_border?> !important;
}
.button:hover, .submit input:hover, .button-secondary:hover, input[type="button"]:hover, input[type="submit"]:hover {
	color: <?Php echo $h_font?>;
	border-color:<?php echo $h_border?>;
...
.button, .submit input, .button-secondary, input[type="button"], input[type="submit"] {
	color:<?php echo $font?>;
