...
//Corner radius		
			...
			(object)array(
				'type'=>'preview',
				'path'=>WLB_URL.'images/preview/login_form/',
				'items'=>array(
					(object)array(
	...
	function css_login_form(){
		...
		foreach(array(
			...
			if(in_array($option, array('lf_bg'))){
				$$option = str_replace("{pluginurl}",WLB_URL,$$option);
...
<?php		
	...
	function login_head(){
		...
		if(trim($login_background)!=''){
			...
			$login_background = str_replace("{pluginurl}",WLB_URL,$login_background);
		...
		if($login_styles_scripts!=''){
			$login_styles_scripts = str_replace("{pluginurl}",WLB_URL,$login_styles_scripts);
		}
?>
<style>
#custom-logo {
